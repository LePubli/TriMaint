...
/* ═══════════════════════════════════════════════════════════════════
   ...
   Mobile-first responsive avec pinch-to-zoom + orientation
   ═══════════════════════════════════════════════════════════════════ */
...
const BOTTOM_NAV_H = 64   // Layout bottom nav h-16 = 64px
const SCHEMA_TOP_BAR_H = 44  // Schema's own top bar
...
export default function SchemaInteractif() {
  ...
  // ─── Detect orientation (mobile only) ────────────────────────
  useEffect(() => {
  ...
  // ─── Compute image display size based on orientation ─────────
  useEffect(() => {
    if (!isMobile || !imgLoaded || imgNatural.w === 0) return

    const updateSize = () => {
      const vw = window.innerWidth
      const vh = window.innerHeight

      // Available height: viewport - layout header - schema top bar - bottom nav - mobile toolbar(48px)
      const mobileToolbarH = 48
      const availH = vh - TOP_HEADER_H - SCHEMA_TOP_BAR_H - BOTTOM_NAV_H - mobileToolbarH
      const availW = vw
      ...
      if (isLandscape) {
        // In landscape: fit height, scale width proportionally
        displayH = Math.max(100, availH)
        displayW = displayH * aspect
        // If it overflows width, fit width instead
        if (displayW > availW) {
          displayW = availW
          displayH = displayW / aspect
        }
      } else {
        // In portrait: fit width, scale height proportionally
        displayW = availW
        displayH = displayW / aspect
        // If it overflows available height, fit height instead
        if (displayH > availH) {
          displayH = availH
          displayW = displayH * aspect
        }
      ...
      setImgDisplaySize({ w: Math.round(displayW), h: Math.round(displayH) })
    }
  ...
  }, [isMobile, isLandscape, imgLoaded, imgNatural.w, imgNatural.h])
  ...
  const handleTouchStart = useCallback((e: React.TouchEvent) => {
    if (e.touches.length === 2) {
      ...
      const dist = Math.sqrt(
        (tx - lastTapPos.current.x) ** 2 + (ty - lastTapPos.current.y) ** 2
      )
      if (timeDelta < 300 && dist < 30) {
        // Double-tap: toggle between 1x and 2.5x centered on tap point
        setZoom(prevZoom => {
          const container = containerRef.current
          if (!container) return prevZoom === 1 ? 2.5 : 1
          const rect = container.getBoundingClientRect()
          const tapX = tx - rect.left
          const tapY = ty - rect.top
          ...
          if (prevZoom > 1.2) {
            ...
            const newZoom = 2.5
            // Calculate pan so tap point stays under finger
            const scale = newZoom / prevZoom
            const newPanX = tapX - (tapX - pan.x) * scale
            const newPanY = tapY - (tapY - pan.y) * scale
  ...
  const imageStyle = useMemo(() => {
    if (isMobile && imgLoaded && imgDisplaySize.w > 0) {
      return { width: imgDisplaySize.w, height: imgDisplaySize.h }
    }
    // Desktop: fill viewport width
    if (imgLoaded) {
      return { width: '100vw', height: 'auto' }
    }
    return {}
  }, [isMobile, imgLoaded, imgDisplaySize])
  ...
  return (
    ...
    `}>
        ...
        <div className="flex items-center gap-1 shrink-0">
          ...
          {!isMobile && (
            <>
              ...
              <span className="text-gray-500 text-[10px] w-10 text-center">
                {Math.round(zoom * 100)}%
              </span>
      ...
      {/* ─── Mobile floating toolbar (above bottom nav) ────────── */}
      {isMobile && (
        <div className="shrink-0 flex items-center justify-center gap-3 px-4 py-2 bg-gray-800/95 backdrop-blur-sm border-t border-gray-700">
          <button
            onClick={zoomOut}
            className="w-10 h-10 rounded-full bg-gray-700 hover:bg-gray-600 active:bg-gray-500 text-gray-200 flex items-center justify-center transition-colors"
            aria-label="Zoom arrière"
          >
            <ZoomOut size={18} />
          </button>
          <span className="text-gray-400 text-xs font-mono w-12 text-center">
            {Math.round(zoom * 100)}%
          </span>
          <button
            onClick={zoomIn}
